 ...
 * Singleton pattern is a design pattern which restricts a class to instanciate its multiple objects. It is nothing but a way of defining a class.
 ...
 * When to Use:
    ...
    -> Prevention of Cloning and Serialization: To maintain the Singleton property, you may need to override the clone method to throw an exception and 
        ensure that the class is not serializable or provide custom serialization logic.
    ...
    -> Permits a variable number of instances: The pattern makesit easy to change your mind and allow more than one instance of the Singleton class. 
        Moreover, you can use the same approach to control the number of instances that the application uses. Only the operation that grants access 
    ...
    -> Singleton Pattern Overuse: Due to its convenience, developers might overuse the Singleton pattern, leading to an abundance of Singleton instances 
        in an application. This can defeat the purpose of the pattern and result in increased memory usage.
    -> Initialization Overhead: Lazy initialization, while often an advantage, can introduce some overhead when the Singleton instance is first accessed. 
        If the initialization process is resource-intensive, it can affect the application’s startup time.
    -> Difficulties in Debugging: Debugging a Singleton-based codebase can be challenging, especially when issues related to the Singleton’s state arise. 
        It can be hard to trace the source of problems when multiple parts of the code may have modified the Singleton’s data.
 */
